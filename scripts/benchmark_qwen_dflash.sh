#!/usr/bin/env bash
# Reproduce the Qwen3.8-27B + DFlash2 server throughput number: start the
# demo's OpenAI-compatible server with fixed synthetic acceptance, wait for it
# to become healthy, then measure the client-visible TTFT / TPOT / end-to-end
# tok/s on ISL=1 / OSL=1000 streaming requests (2 warmup + 12 timed).
#
# Usage:
#   JOBID=<slurm-job> bash scripts/benchmark_qwen_dflash.sh
#
# PORT, CONTEXT, MAX_TOKENS, ACCEPT (accepted drafts per round), STEPS_PER_CALL,
# ISL, OSL, N_WARMUP, N_BENCH, STARTUP_TIMEOUT, and OUTPUT_PATH may be
...
ACCEPT="${ACCEPT:-5}"          # accepted drafts per round (acceptance length 6)
...
OUTPUT_PATH="${OUTPUT_PATH:-${REPO_ROOT}/benchmark-results/qwen-dflash-${RUN_ID}}"
...
echo "Server ready; benching ${N_BENCH} timed requests (ISL=${ISL} OSL=${OSL}, acceptance $((ACCEPT + 1)))"
